...
(function (IceDevQr) {
    const QrCode = /** @class */ (function () {
        function QrCode(instance) {
            ...
            this.allIceElement = document.getElementsByClassName("iceBitsyPayRow");
            this.containerElement.innerHTML = ''

            for (let i = 0; i < this.allIceElement.length; ++i) {
                let item = this.allIceElement[i];
                if (item.name == 'shopAuthCode') this.shopAuthCode = item.value
                if (item.name == 'shopUid') this.shopUid = item.value
                if (item.name == 'amount') this.amount = item.value
                if (item.name == 'order') this.order = item.value
                if (item.name == 'description') this.description = item.value
                if (item.name == 'name') this.name = item.value
                if (item.name == 'email') this.email = item.value
                if (item.name == 'phone') this.phone = item.value
                if (item.name == 'currency') this.currency = item.value
            }
        ...
        QrCode.prototype.init = function () {
            ...
            xhr.onreadystatechange = function () // Ждём ответа от сервера
            {
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) {
                        this.qrResponse = JSON.parse(xhr.responseText)
    ...
    const Api = /** @class */ (function () {
        ...
        Api.prototype.qrCode = function () {
            ...
            if (this.payButton.type === 'submit') return;
            let widget = new QrCode(this);
            widget.init();
            return widget;
        };
        ...
        Api.prototype.runInitCallbacks = function () {
            var iceDevApiInitCallbacks = window.iceDevApiInitCallbacks;
            if (iceDevApiInitCallbacks && iceDevApiInitCallbacks.length) {
                setTimeout(function () {
                    var callback;
                    while (callback = iceDevApiInitCallbacks.shift()) {
                        try {
                            callback();
                        } catch (e) {
                            console.error(e);
...
if (typeof window['iceDevApi'] === 'undefined') {
    window.iceDevApi = new IceDevQr.Api();
    window.iceDevApi.runInitCallbacks();
}
